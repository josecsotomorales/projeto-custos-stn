SELECT TO_CHAR(datum, 'yyyymmdd')::INT AS id,
       ...
       EXTRACT(ISODOW FROM datum)::INT AS dia_da_semana,
       EXTRACT(DAY FROM datum)::INT AS dia_do_mes,
       datum - DATE_TRUNC('quarter', datum)::DATE + 1 AS dia_do_trimestre,
       EXTRACT(DOY FROM datum)::INT AS dia_do_ano,
       TO_CHAR(datum, 'W')::INT AS semana_do_mes,
       EXTRACT(WEEK FROM datum)::INT AS semana_do_ano,
       EXTRACT(ISOYEAR FROM datum) || TO_CHAR(datum, '"-W"IW-') || EXTRACT(ISODOW FROM datum) AS semana_do_ano_iso,
       EXTRACT(MONTH FROM datum)::INT AS mes_atual,
       TO_CHAR(datum, 'Month') AS mes_nome,
       TO_CHAR(datum, 'Mon') AS mes_nome_abreviado,
       EXTRACT(QUARTER FROM datum)::INT AS trimestre_atual,
       CASE
       ...
       EXTRACT(ISOYEAR FROM datum)::INT AS ano_atual,
