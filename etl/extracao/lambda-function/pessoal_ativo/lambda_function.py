...
def get_s3_file_content(s3_path):
    ...
    if not is_s3_file_exists(s3_path):
        if s3_path == CUSTO_STN_SOURCES_STATES:
            ...
            if content:
                load_s3_file_content(s3_path, json.dumps(content,indent = 2) )
            else:
...
def consume_api(custos_stn_state,pessoal_ativo):
    ...
    if len(pessoal_ativo_items['items']) > 0:
        ...
        load_s3_file_content(CUSTO_STN_SOURCES_STATES, json.dumps(custos_stn_state, indent= 2))
        load_s3_file_content(f'{CUSTO_STN_PESSOAL_ATIVO_PATH}/pessoal_ativo_{pessoal_ativo.file_number}', json.dumps(pessoal_ativo_items['items'], indent= 2))

    while pessoal_ativo_items['hasMore'] and pessoal_ativo_items['initial_offset'] <= 750:
